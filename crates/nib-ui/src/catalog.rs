...
pub fn build_generate_command(generator: Arc<dyn Generator>) -> CommandDef {
    let mut generate = CommandDef::build("generate", GenerateHandler { generator })
        ...
        .mcp(McpCommandOptions {
            ...
            instructions: Some(
                "Sign in with `nib auth login`. Provide a precise UI brief and up to three PNG, JPEG, or WebP references as data URIs. Fast 1K is the default and is eligible for the one-image free trial. Return the image directly to the user."
                    .to_string(),
            ),
