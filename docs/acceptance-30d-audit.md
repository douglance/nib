...
| Three published end-to-end acceptance examples | Example recipe work is expected under `examples/acceptance/{onboarding,business-rules,permissions}` and companion self-serve instructions. Local runtime validation can prove the examples execute against a configured local service. The current example source is still being corrected and this audit does not lock example commands. | Three production review URLs or exported receipts created from the published examples, each tied to a real project, commit, manifest hash, reviewer action, verification result, and evidence URL. | Not complete. Local examples and smoke output do not prove published production examples. |
...
| Published examples | Three review URLs or exported packets, manifest hashes, commits, reviewer decisions, verification receipts, and evidence URLs. Do not record command names until the example source and instructions are finalized. |
...
- The current source validation is tied to commit `11a1d59` and local worktree changes. Treat it as source evidence until a pushed commit and production deployment are recorded.
