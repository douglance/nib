...
The first sales-page visual in `site/src/generated-ui-hero.png` came from Visualize through Cloudflare's `google/nano-banana-2` model. The asset build copies that image into the public Worker's static asset payload. The page around it is rendered on each request by the Topcoat Rust/Wasm Worker.
...
npx wrangler d1 migrations apply visualize --local --config wrangler.dogfood.jsonc
npx wrangler d1 execute visualize --local --config wrangler.dogfood.jsonc \
  --command "INSERT OR IGNORE INTO accounts(tenant_id, plan, created_at, updated_at) VALUES ('dogfood@visualize.local', 'default', unixepoch(), unixepoch())"
```
...
`wrangler.dogfood.jsonc` sets `ENVIRONMENT=development`, so the local-only `x-visualize-dev-tenant` header can select the isolated tenant. Production ignores that header.

## Generate

```sh
VISUALIZE_BACKEND_URL=http://127.0.0.1:8790/internal/v1/generate \
VISUALIZE_DEV_TENANT=dogfood@visualize.local \
visualize generate \
...
| `GET /health` | `200` |
| `GET /` | `200` |
| `GET /` response header | `x-visualize-renderer: topcoat-wasm-worker` |
