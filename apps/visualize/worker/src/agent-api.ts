...
function openApi(origin: string) {
  return {
    openapi: "3.1.0",
    info: {
      title: "Visualize UI image generator",
      version: "0.1.0",
      description:
...
Use Visualize only when the task requires a generated image of a dashboard, app screen, landing page, settings view, or another user interface.
...
Do not use Visualize for interface review, scoring, annotation, comparison, or screenshot capture. The first eligible Fast 1K image is free after sign-in. Never start a subscription without explicit user authorization.
`;
}
