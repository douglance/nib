...
export function mcpResponse(
  ...
  const tenantId = request.headers.get("x-visualize-tenant");
  const trialNetwork = request.headers.get("x-visualize-trial-network");
  ...
  server.registerTool(
    ...
    async (arguments_: GenerateArguments) => {
      ...
      const headers = new Headers({
        ...
        "x-visualize-tenant": tenantId,
      });
      if (trialNetwork) headers.set("x-visualize-trial-network", trialNetwork);
      const response = await handleGeneration(
        new Request("https://worker.visualize/internal/v1/generate", {
          method: "POST",
          headers,
      ...
      if (!response.ok) {
        return toolError(
          ...
          String(result.message ?? "Visualize could not generate the UI image."),
        );
      }
