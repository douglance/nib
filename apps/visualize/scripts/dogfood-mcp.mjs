...
const endpoint = process.env.VISUALIZE_MCP_URL ?? "http://127.0.0.1:8790/mcp";
const tenant = process.env.VISUALIZE_DEV_TENANT ?? "dogfood@visualize.local";
const resumeJob = process.argv[2] ?? process.env.VISUALIZE_RESUME_JOB;
let sessionId;
...
const initialized = await rpc(1, "initialize", {
  ...
  capabilities: {},
  clientInfo: { name: "visualize-dogfood", version: "1.0.0" },
});
