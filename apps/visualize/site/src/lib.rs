...
const INSTALL_PROMPT: &str = r#"Install Visualize for me. Follow https://visualize.doug-lance.workers.dev/install-agent.md exactly. Configure it globally for this agent, install the Visualize UI image skill globally, add the managed Visualize instruction to this agent's global instruction file, preserve my existing settings, and verify that the generate_ui tool is available without generating an image."#;
...
const SEO_JSON_LD: &str = r#"{
  ...
  "@graph": [
    {
      "@type": "WebSite",
      "@id": "https://visualize.doug-lance.workers.dev/#website",
      "url": "https://visualize.doug-lance.workers.dev/",
      "name": "Visualize",
      "description": "A UI image generator for AI agents and developer tools."
    },
    {
      "@type": "SoftwareApplication",
      "@id": "https://visualize.doug-lance.workers.dev/#software",
      "name": "Visualize",
      "url": "https://visualize.doug-lance.workers.dev/",
...
async fn home() -> Result {
    view! {
        page_document(
            title: "UI image generator for AI agents | Visualize",
            description: "Generate a UI image from a prompt and optional references. Use Visualize when your model, coding agent, or workflow can describe an interface but cannot render it.",
            canonical: "https://visualize.doug-lance.workers.dev/",
            <main>
                <section class="shell hero">
                    <div>
                        <div class="eyebrow">"UI image generator"</div>
                        <h1>"Generate a UI image from a prompt."</h1>
                        <p class="lede">"Describe the dashboard, app screen, landing page, or interface you need. Visualize returns one finished PNG or JPEG, even when the model or agent you are using cannot generate images."</p>
                        <div class="actions">
                            <a class="button primary" href="#agents">"Install Visualize free"</a>
...
async fn pricing_page() -> Result {
    view! {
        page_document(
            title: "UI image generation pricing | Visualize",
            description: "Pricing for the Visualize UI image generator. Try one eligible Fast 1K image free, then choose a plan and pay for generation usage.",
            canonical: "https://visualize.doug-lance.workers.dev/pricing",
            <main>pricing()</main>
        )
    }
}
...
async fn account() -> Result {
    view! {
        page_document(
            title: "Visualize account",
            description: "Manage your Visualize subscription and UI image generation plan.",
            canonical: "https://visualize.doug-lance.workers.dev/account",
            <main class="shell legal">
                <div class="eyebrow">"Account"</div>
                <h1>"Manage Visualize."</h1>
...
async fn docs() -> Result {
    view! {
        page_document(
            title: "Generate UI images with MCP, CLI, or API | Visualize",
            description: "Connect an AI agent to the Visualize UI image generator with MCP, CLI, HTTP, OpenAPI, or an installable skill.",
            canonical: "https://visualize.doug-lance.workers.dev/docs",
            <main class="shell docs">
                ...
                <pre>"cloudflared access login https://visualize.doug-lance.workers.dev/internal/v1/generate\nexport VISUALIZE_ACCESS_TOKEN=\"$(cloudflared access token -app=https://visualize.doug-lance.workers.dev/internal/v1/generate)\""</pre>
                <h2>"CLI"</h2>
                <pre>"visualize generate \"A compact dark analytics dashboard for a fleet operator\" \\\n  --quality fast --resolution 1K --aspect 16:9 \\\n  --image-format png --output dashboard.png"</pre>
                <h2 id="mcp">"MCP"</h2>
                <p>"For local stdio MCP, run " <code>"visualize --mcp"</code> " with " <code>"VISUALIZE_ACCESS_TOKEN"</code> " set. A Streamable HTTP client connects to " <code>"https://visualize.doug-lance.workers.dev/mcp"</code> " without authentication for installation and tool discovery. Before the first image, add the user-scoped token as " <code>"cf-access-jwt-assertion"</code> ". The server exposes exactly one tool: " <code>"generate_ui"</code> "."</p>
                ...
                <pre>"https://visualize.doug-lance.workers.dev/openapi.json\nhttps://visualize.doug-lance.workers.dev/.well-known/skills/generate/SKILL.md"</pre>
                <h2>"References"</h2>
                <p>"Pass up to three PNG, JPEG, or WebP files with repeated " <code>"--ref"</code> " options. Visualize deletes temporary references after the generation attempt."</p>
            </main>
        )
    }
}
    ...
    view! {
        ...
        <html lang="en">
            <head>
                ...
                <meta property="og:site_name" content="Visualize">
...
async fn site_nav() -> Result {
    view! {
        <header class="shell nav">
            <a class="brand" href="/">"visualize" <span class="brand-dot">"."</span></a>
            <nav class="nav-links" aria-label="Primary navigation">
                <a href="/#agents">"For agents"</a>
                <a href="/#pricing">"Pricing"</a>
                <a class="button" href="/#agents">"Install Visualize"</a>
            </nav>
        </header>
    }
}
...
async fn ui_viewport() -> Result {
    view! {
        <div class="viewport">
            <img src="/assets/generated-ui-hero.png" alt="Dark Visualize dashboard showing API requests, compute hours, a generation-volume chart, and recent image jobs.">
        </div>
    }
}
...
async fn how_it_works() -> Result {
    view! {
        <section class="section" id="how"><div class="shell">
            ...
            <p class="section-copy">"Use Visualize to turn a dashboard, settings screen, mobile app, landing page, or another user interface into an image. It accepts the brief and returns one generated viewport."</p>
            <div class="steps">
                <article class="step"><b>"01 / FIND"</b><h3>"Choose the image tool"</h3><p>"Visualize is public and discoverable before you create an account or provide payment details."</p></article>
                ...
                <article class="step"><b>"03 / GET"</b><h3>"Receive one image"</h3><p>"Visualize returns a PNG or JPEG to save, show, or use in the next task."</p></article>
            </div>
        </div></section>
    }
}
...
async fn agent_connection() -> Result {
    view! {
        <section class="section" id="agents"><div class="shell">
            ...
            <p class="section-copy">"Paste the prompt into Codex, Claude Code, Gemini CLI, or another coding agent. It connects the remote MCP tool globally, installs the Visualize skill, and teaches the root agent when to use it."</p>
            install_prompt()
            <div class="install-facts" aria-label="Installation results">
                ...
                <div><strong>"One managed instruction"</strong>"Existing agent instructions stay intact and the Visualize block is added once."</div>
            </div>
        </div></section>
    }
}
...
async fn site_footer() -> Result {
    view! { <footer class="shell footer"><span>"Visualize - UI image generation from one prompt."</span><span><a href="/privacy">"Privacy"</a> "  /  " <a href="/terms">"Terms"</a></span></footer> }
}
