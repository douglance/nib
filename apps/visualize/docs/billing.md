...
An eligible Cloudflare Access identity receives one blocking Fast 1K image without a card. Trial artifacts expire after 1 day. Trial generations do not create Stripe meter events.
...
The service records usage only after a successful image fetch and R2 write. `usage_cents` is a whole-number Stripe meter value, so the meter is named `visualize_usage_cents`.
...
3. A meter named `visualize_usage_cents` with sum aggregation.
