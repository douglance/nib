...
- Transport: Streamable HTTP at `/mcp`, or stdio with `visualize --mcp`.
...
Authenticate once under the end user's identity, then export the application token for the Visualize CLI and local stdio MCP server:
...
Only Worker-internal routing may supply `x-visualize-tenant`. The Worker deletes any caller-provided tenant header and replaces it with the verified identity before invoking generation; see [`worker/src/access.ts`](../worker/src/access.ts) and [`worker/src/index.ts`](../worker/src/index.ts).
...
```json
{
  ...
  "format": "png",
  "usage_cents": 32,
  "artifact_url": "https://visualize.example.com/artifacts/uuid",
  "image": {
    "data": "base64...",
    "mime_type": "image/png"
  },
...
| Remote MCP skill | `/install/visualize-ui-image/SKILL.md` |
