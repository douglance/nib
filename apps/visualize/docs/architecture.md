...
Visualize production is a Cloudflare Workers application. It does not use Cloudflare Containers, a hosted Rust server, or Browser Rendering.
...
Topcoat is a production request-time renderer. `visualize-topcoat` compiles to `wasm32-unknown-unknown` with workers-rs and calls `Router::handle` for each page request. The public `visualize` Worker reaches it through the `SITE` service binding. `npm run site:build` still creates deterministic HTML snapshots for build validation and copies the image and browser script into `site/dist/assets`; production page routes do not serve those HTML snapshots. The local Rust CLI is a client and optional stdio MCP transport.
