...
mod tests {
    ...
    fn test_list_annotations() {
        ...
        let box_ann = Annotation::new(AnnotationType::Box {
            region: Region::new(10.0, 20.0, 50.0, 30.0),
            stroke_width: 2.0,
            stroke_style: StrokeStyle::Solid,
            filled: false,
        ...
        let arrow_ann = Annotation::new(AnnotationType::Arrow {
        ...
        nib.add_annotation(&box_ann).unwrap();
        nib.add_annotation(&arrow_ann).unwrap();

        let annotations = nib.list_annotations().unwrap();
    ...
    fn test_update_annotation() {
        ...
        nib.update_annotation(&id, &updated).unwrap();

        // Verify
        let retrieved = nib.get_annotation(&id).unwrap().unwrap();
        assert_eq!(retrieved.color.r, Color::BLUE.r);
    }
    ...
    fn test_delete_annotation() {
        ...
        assert!(nib.delete_annotation(&id).unwrap());
        assert!(!nib.delete_annotation(&id).unwrap()); // Second delete returns false

        assert!(nib.get_annotation(&id).unwrap().is_none());
    }
    ...
    fn test_all_annotation_types() {
        ...
        let types = vec![
            ...
            AnnotationType::Box {
                region: Region::new(10.0, 10.0, 40.0, 40.0),
                stroke_width: 2.0,
            ...
            AnnotationType::Highlight {
                region: Region::new(0.0, 0.0, 100.0, 20.0),
                corner_radius: 4.0,
            },
            AnnotationType::Blur {
                region: Region::new(50.0, 50.0, 30.0, 30.0),
            ...
            AnnotationType::Crop {
                region: Region::new(10.0, 10.0, 80.0, 80.0),
            },
        ];

        for ann_type in types {
            let annotation = Annotation::new(ann_type);
            let id = nib.add_annotation(&annotation).unwrap();

            let retrieved = nib.get_annotation(&id).unwrap().unwrap();
            assert_eq!(
                annotation.annotation_type.type_name(),
                retrieved.annotation_type.type_name()
            );
        }

        assert_eq!(nib.annotation_count().unwrap(), 9);
    }
    ...
    fn test_metadata() {
        ...
        nib.set_metadata("title", "Test Image").unwrap();
        nib.set_metadata("author", "Test User").unwrap();

        assert_eq!(
            nib.get_metadata("title").unwrap(),
            Some("Test Image".to_string())
        );
        assert_eq!(
            nib.get_metadata("author").unwrap(),
            Some("Test User".to_string())
        );
        assert_eq!(nib.get_metadata("nonexistent").unwrap(), None);

        // Update metadata
        nib.set_metadata("title", "Updated Title").unwrap();
        assert_eq!(
            nib.get_metadata("title").unwrap(),
    ...
    fn test_session() {
        ...
        assert!(!nib.is_gui_open().unwrap());

        nib.update_session(Some(12345)).unwrap();
        assert!(nib.is_gui_open().unwrap());

        nib.clear_session().unwrap();
        assert!(!nib.is_gui_open().unwrap());
    }
    ...
    #[test]
    fn test_list_annotations_since() {
        let temp_dir = TempDir::new().unwrap();
        let path = temp_dir.path().join("test.nib");
        let image_data = create_test_image();
        ...
        // Record timestamp before first annotation
        let before_first = system_time_to_unix(SystemTime::now());
        ...
        // Record timestamp after first annotation
        let after_first = system_time_to_unix(SystemTime::now());

        // Query since before first - should find one
        let since_before = nib.list_annotations_since(before_first - 1).unwrap();
        assert_eq!(since_before.len(), 1);

        // Query since after first - should find none (created at same second or before)
        let since_after = nib.list_annotations_since(after_first + 1).unwrap();
        assert_eq!(since_after.len(), 0);

        // Test latest_annotation_modified_at
        let latest = nib.latest_annotation_modified_at().unwrap();
    ...
    fn test_get_session() {
        ...
        let session = nib.get_session().unwrap();
        ...
        let (pid, opened_at, _) = session.unwrap();
    ...
    fn test_ocr_cache_full_image() {
        ...
        // Initially no cache
        assert!(nib.get_cached_ocr(None).unwrap().is_none());
        assert!(!nib.is_ocr_cached(None, 3600).unwrap());
        ...
        nib.cache_ocr(None, &entries).unwrap();

        // Verify cache exists
        assert!(nib.is_ocr_cached(None, 3600).unwrap());

        // Retrieve cached entries
        let cached = nib.get_cached_ocr(None).unwrap().unwrap();
        assert_eq!(cached.len(), 2);
        assert_eq!(cached[0].text, "Hello");
        assert_eq!(cached[0].x, 10);
        assert_eq!(cached[0].y, 20);
        assert_eq!(cached[0].width, 50);
        assert_eq!(cached[0].height, 15);
        assert!((cached[0].confidence - 0.95).abs() < 0.01);
        assert_eq!(cached[1].text, "World");
    }

    #[test]
    fn test_ocr_cache_with_region() {
        let temp_dir = TempDir::new().unwrap();
        let path = temp_dir.path().join("test.nib");
        let image_data = create_test_image();
        ...
        // Add entries for full image
        let full_entries = vec![OcrCacheEntry {
            text: "Full".to_string(),
            x: 0,
            y: 0,
            width: 40,
        ...
        nib.cache_ocr(None, &full_entries).unwrap();

        // Add entries for specific region
        let region_entries = vec![OcrCacheEntry {
        ...
        nib.cache_ocr(Some("50,50,100,100"), &region_entries).unwrap();

        // Verify both caches are separate
        let full_cached = nib.get_cached_ocr(None).unwrap().unwrap();
        assert_eq!(full_cached.len(), 1);
        assert_eq!(full_cached[0].text, "Full");

        let region_cached = nib.get_cached_ocr(Some("50,50,100,100")).unwrap().unwrap();
        assert_eq!(region_cached.len(), 1);
        assert_eq!(region_cached[0].text, "Region");

        // Non-existent region returns None
        assert!(nib.get_cached_ocr(Some("0,0,10,10")).unwrap().is_none());
    }

    #[test]
    fn test_ocr_cache_clear() {
        let temp_dir = TempDir::new().unwrap();
        let path = temp_dir.path().join("test.nib");
        let image_data = create_test_image();
        ...
        // Cache for full image and a region
        nib.cache_ocr(None, &entries).unwrap();
        nib.cache_ocr(Some("10,10,50,50"), &entries).unwrap();

        // Clear only full image cache
        nib.clear_ocr_cache(None).unwrap();
        assert!(nib.get_cached_ocr(None).unwrap().is_none());
        assert!(nib.get_cached_ocr(Some("10,10,50,50")).unwrap().is_some());

        // Clear all
        nib.clear_all_ocr_cache().unwrap();
        assert!(nib.get_cached_ocr(Some("10,10,50,50")).unwrap().is_none());
    }

    #[test]
    fn test_ocr_cache_expiry() {
        let temp_dir = TempDir::new().unwrap();
        let path = temp_dir.path().join("test.nib");
        let image_data = create_test_image();
        ...
        // Fresh cache (within 1 hour)
        assert!(nib.is_ocr_cached(None, 3600).unwrap());

        // Very short max_age - cache is still valid if just created
        // Note: max_age=0 means "cached_at must be >= now", which is true for just-created entries
        assert!(nib.is_ocr_cached(None, 1).unwrap());

        // Very large negative offset would make it "expired" but we use positive max_age
        // Test that a reasonable max_age works
        assert!(nib.is_ocr_cached(None, 60).unwrap()); // Within 60 seconds
    }
    ...
    fn test_grid_cache() {
        ...
        // Initially no cache
        assert!(nib.get_cached_grid(100, None).unwrap().is_none());
        ...
        let metadata = serde_json::json!({
            ...
            "grid_size": [1, 1],
            "major_interval": 5,
            "formula": "pixel_x = origin[0] + col * spacing"
        });

        nib.cache_grid(100, None, &metadata).unwrap();

        // Retrieve cached grid
        let cached = nib.get_cached_grid(100, None).unwrap().unwrap();
        assert_eq!(cached["spacing"], 100);
        assert_eq!(cached["grid_size"][0], 1);

        // Different spacing returns None
        assert!(nib.get_cached_grid(50, None).unwrap().is_none());
    }

    #[test]
    fn test_grid_cache_with_region() {
        let temp_dir = TempDir::new().unwrap();
        let path = temp_dir.path().join("test.nib");
        let image_data = create_test_image();
        ...
        let full_metadata = serde_json::json!({"type": "full"});
        let region_metadata = serde_json::json!({"type": "region"});

        nib.cache_grid(100, None, &full_metadata).unwrap();
        nib.cache_grid(100, Some("0,0,50,50"), &region_metadata).unwrap();

        // Both cached separately
        let full = nib.get_cached_grid(100, None).unwrap().unwrap();
        assert_eq!(full["type"], "full");

        let region = nib.get_cached_grid(100, Some("0,0,50,50")).unwrap().unwrap();
        assert_eq!(region["type"], "region");
    }

    #[test]
    fn test_grid_cache_clear() {
        let temp_dir = TempDir::new().unwrap();
        let path = temp_dir.path().join("test.nib");
        let image_data = create_test_image();
        ...
        nib.cache_grid(100, None, &metadata).unwrap();
        nib.cache_grid(50, None, &metadata).unwrap();

        assert!(nib.get_cached_grid(100, None).unwrap().is_some());
        assert!(nib.get_cached_grid(50, None).unwrap().is_some());

        nib.clear_grid_cache().unwrap();
    ...
    fn test_render_cache() {
        ...
        // Calculate hash with no annotations
        let hash = nib.annotations_hash().unwrap();
        assert!(!hash.is_empty());
        assert_eq!(hash.len(), 64); // SHA256 hex is 64 chars

        // Initially no cache
        assert!(nib.get_cached_render(&hash).unwrap().is_none());

        // Cache rendered data
        let rendered_data = vec![1, 2, 3, 4, 5];
        nib.cache_render(&rendered_data, &hash).unwrap();

        // Retrieve with matching hash
        let cached = nib.get_cached_render(&hash).unwrap().unwrap();
        assert_eq!(cached, rendered_data);

        // Different hash returns None
        assert!(nib.get_cached_render("wrong_hash").unwrap().is_none());
    }
    ...
    fn test_render_cache_invalidation() {
        ...
        // Get initial hash (no annotations)
        let hash1 = nib.annotations_hash().unwrap();

        // Cache render
        let rendered = vec![10, 20, 30];
        nib.cache_render(&rendered, &hash1).unwrap();
        ...
        // Hash should change
        let hash2 = nib.annotations_hash().unwrap();
        assert_ne!(hash1, hash2);

        // Old cache is no longer valid
        assert!(nib.get_cached_render(&hash2).unwrap().is_none());

        // But can still retrieve with old hash
        let old_cached = nib.get_cached_render(&hash1).unwrap().unwrap();
        assert_eq!(old_cached, rendered);
    }

    #[test]
    fn test_render_cache_update() {
        let temp_dir = TempDir::new().unwrap();
        let path = temp_dir.path().join("test.nib");
        let image_data = create_test_image();
        ...
        // Cache first version
        nib.cache_render(&[1, 2, 3], &hash).unwrap();

        // Update with new data (same hash - simulating re-render)
        nib.cache_render(&[4, 5, 6], &hash).unwrap();

        // Should get the updated data
        let cached = nib.get_cached_render(&hash).unwrap().unwrap();
        assert_eq!(cached, vec![4, 5, 6]);
    }
    ...
    fn test_render_cache_clear() {
        ...
        assert!(nib.get_cached_render(&hash).unwrap().is_some());

        nib.clear_render_cache().unwrap();

        assert!(nib.get_cached_render(&hash).unwrap().is_none());
    }

    #[test]
    fn test_annotations_hash_deterministic() {
        let temp_dir = TempDir::new().unwrap();
        let path = temp_dir.path().join("test.nib");
        let image_data = create_test_image();
    ...
    #[test]
    fn test_annotations_hash_changes_with_color() {
        let temp_dir = TempDir::new().unwrap();
        let path = temp_dir.path().join("test.nib");
        let image_data = create_test_image();
